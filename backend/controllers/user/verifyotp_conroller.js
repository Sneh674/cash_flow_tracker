...
const verifyOTP = async (req, res, next) => {
  ...
  if (!otpgenerated) {
    return next(createHTTPError(500, "Error generating otp"));
  }
  if (!otpentered) {
    return next(createHttpError(400, "otp required"));
  }

  if (otpentered !== otpgenerated) {
    return next(createHTTPError(400, "incorrect otp"));
  }

  try {
    const user = await userModel.findOne({ _id: userId });
    if (!user) {
      return next(createHTTPError(500, "User id not found"));
    }
  } catch (error) {
  ...
  try {
    try {
      ...
      return next(
        createHTTPError(500, "Error updating whether user is verified or not")
      );
    }
